...
Yolov3 :: Yolov3()
{
    L().info()<<"YOLOV3 Constructor "<< endl;
    custom_postprocess = NULL;
}

Yolov3 :: ~Yolov3()
{
    L().info() << "YOLOV3 Destructor" << endl;
}

void Yolov3 :: set_target_env(deie_detector *detobj, deie_hw_platform tgt, deie_device device)
{
...
void Yolov3 :: set_target_input(deie_detector *detobj, deie_input_src input, deie_file_input *path)
{   
    switch (input)
    {           
...
//for camera input
void Yolov3 :: set_target_input(deie_detector *detobj, deie_input_src input, int devicefd) 
{
    switch(input)
    {
...
void Yolov3 :: set_model_info (deie_detector *detobj, deie_detector_model model)
{
...
void Yolov3 :: set_input_pre_processing(deie_detector *detobj, deie_detector_input_preproc preproc, deie_input_color_format cfmt)
{

}
void Yolov3 :: set_input_pre_processing(deie_detector *detobj, deie_detector_input_preproc preproc, void *value)
{

}

void Yolov3 :: set_output_buffer(deie_detector *detobj, void *ptr, int size)
{

}
void Yolov3 :: set_output_params(deie_detector *detobj, deie_detector_output_params outparams, int value)
{

}

void Yolov3 :: allocate_io_buffer(deie_detector *detobj, deie_io_desc *ioptr)
{
...
void Yolov3 :: free_io_buffer(deie_detector *detobj, deie_io_desc *ioptr)
{
    //free(ioptr);
}

void Yolov3 :: set_model_input_buffers(deie_detector *detobj, int num, void** ptr)
{

}

void Yolov3 :: set_model_output_buffers(deie_detector *detobj, int num, void** ptr)
{

}

void Yolov3 :: register_custom_pre_processor(deie_detector *detobj, deie_detector_preprocessor preprocessor)
{

}
void Yolov3 :: register_custom_post_processor(deie_detector *detobj, deie_detector_postprocessor postprocessor)
{
    if (detobj == this)
        custom_postprocess = postprocessor;
}

void Yolov3 :: intel_nuc_post_process(void *, int num_outputs, deie_detector_model* out)
{   
    ...
    float *boxes = (float *)out_node[3].data;
    cout<<"boxes from ptr void: "<<*boxes<<endl;
    ...
    process_out.payload["boxes"] = {};
    process_out.payload["scores"] = {};
    ...
    {     
        ...
        if ( max_score > 0.75){
            ...
            vector<float> det_bbox;
            det_bbox.push_back(static_cast<float>(boxes[i * 4 + 0]));
            det_bbox.push_back(static_cast<float>(boxes[i * 4 + 1]));
            det_bbox.push_back(static_cast<float>(boxes[i * 4 + 2]));
            det_bbox.push_back(static_cast<float>(boxes[i * 4 + 3]));
            vector<float> det_scores(&scores[start], &scores[end]);
            process_out.payload["boxes"].push_back(det_bbox);
            process_out.payload["scores"].push_back(det_scores);
            // detections[4] = static_cast<float>(scores[i * 4 + 0]);
            // detections[5] = static_cast<float>(scores[i * 4 + 1]);
            // detections[6] = static_cast<float>(scores[i * 4 + 2]);
            // detections[7] = static_cast<float>(scores[i * 4 + 3]);
            //int label_pos = distance(scores, max_element(&scores[start], &scores[end]));
            //std::cout << label_pos <<endl;
            // printf("\nStart: %d | End: %d \n | ", file_name.c_str(), start, end);
...
void Yolov3 :: run_inference(deie_detector *detobj, deie_detector_process_results inference_out)
{
...
void Yolov3 :: stop_inference(deie_detector *detobj)
{
    delete detobj;
}
