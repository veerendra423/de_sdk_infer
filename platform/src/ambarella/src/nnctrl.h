...
/*!
 *  @brief define nnctrl library supported update_pitch aligned size
 */
#define PORT_UPDATE_PITCH_ALIGN (32)
...
struct net_cfg {
	...
	IN uint32_t enable_dvi_crc_check : 1;  /*!< Set it as 1 will calculate dvi's CRC32 value when load_net,
		* and check if its value changed after encounter VP Hang.
		* Use this method to check if VP Hang caused by broken DVI file. */
	uint32_t reserved_0 : 7;  /*!< Reserved field */
...
struct input_desc {
	...
	IN uint32_t update_pitch : 16;  /*!< Run-time changes the pitch of the input.
		...
		* Can not less than width size, at least be @PORT_UPDATE_PITCH_ALIGN byte align. */
...
/*!
 * @brief Detailed port index information in @struct cavalry_run_dags
 */
struct io_index_cfg {
	OUT const char *name;  /*!< Port name that get from library */
	OUT unsigned long ucmd_phys;  /*!< The phsical address for struct dag_run_cmd */
	OUT uint32_t dvi_id;  /*!< DVI index in @struct cavalry_run_dags */
	OUT uint32_t port_id;  /*!< Port index in @struct cavalry_run_dags */
	OUT uint32_t pitch_poke_id;  /*!< Poke index for update_pitch in @struct cavalry_run_dags */

	uint32_t reserved[5];  /*!< Reserved field */
};

/*!
 * @brief Total network input and output's configuration in @struct cavalry_run_dags
 */
struct net_io_rundag_cfg {
	OUT uint32_t num;  /*!< Network Intput or Output number, get from library */
	OUT struct io_index_cfg desc[MAX_IO_NUM];  /*!< Network Intput or Output descriptor */
...
/*!
 * This API allows user to query index information of network input and ouput
 * in @struct cavalry_run_dags.
 * @param net_id the network ID
 * @param in the pointer to input of network. Can be NULL
 * @param out the pointer to output of network. Can be NULL
 * @return 0 = success, -1 = error.
 */
AMBA_API int nnctrl_query_net_rundag_cfg(IN int net_id,
	OUT struct net_io_rundag_cfg *input, OUT struct net_io_rundag_cfg *output);
